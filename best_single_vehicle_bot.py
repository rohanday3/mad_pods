...
while True:
    ...
    # 4b. Multi-step Collision prediction & Blocking adjustment
    t_collision = None
    for t in [1, 2, 3]:
        # Simple linear extrapolation of positions
        extrap_our_x = x + vx * t
        extrap_our_y = y + vy * t
        extrap_opp_x = opp_x + opp_vx * t
        extrap_opp_y = opp_y + opp_vy * t
        
        d_t = math.hypot(extrap_our_x - extrap_opp_x, extrap_our_y - extrap_opp_y)
        if d_t < 950:
            t_collision = t
            break
            
    if t_collision is not None:
        extrap_our_x = x + vx * t_collision
        extrap_our_y = y + vy * t_collision
        opp_x_t = opp_x + opp_vx * t_collision
        opp_y_t = opp_y + opp_vy * t_collision
        
        opp_target_dx = cx - opp_x_t
        opp_target_dy = cy - opp_y_t
        opp_target_d = math.hypot(opp_target_dx, opp_target_dy)
        
        bounce_dx = opp_x_t - extrap_our_x
        bounce_dy = opp_y_t - extrap_our_y
        bounce_d = math.hypot(bounce_dx, bounce_dy)
        
        is_good_collision = False
        if opp_target_d > 0 and bounce_d > 0:
            dot_product = (bounce_dx * opp_target_dx + bounce_dy * opp_target_dy) / (bounce_d * opp_target_d)
            # A negative dot product means the bounce direction points away from the checkpoint
            if dot_product <= 0.0:
                is_good_collision = True
                
        opp_dist = math.hypot(cx - opp_x, cy - opp_y)
        if dist < opp_dist and is_good_collision:
            # We are ahead and it's a good collision: block them
            if opp_target_d > 0:
                ux, uy = opp_target_dx / opp_target_d, opp_target_dy / opp_target_d
            else:
                ux, uy = 0.0, 0.0
            block_x = opp_x_t + ux * 750
            block_y = opp_y_t + uy * 750
            blend = 0.45 if t_collision == 1 else (0.28 if t_collision == 2 else 0.15)
            target_x = target_x * (1.0 - blend) + block_x * blend
            target_y = target_y * (1.0 - blend) + block_y * blend
        else:
            # We are behind or it's a bad collision: steer away to dodge them completely
            if t_collision <= 2:
                escape_x = extrap_our_x - opp_x_t
                escape_y = extrap_our_y - opp_y_t
    ...
    if dist_next_opp < 800 and shield_cooldown == 0:
        ...
        if math.hypot(next_our_vx - opp_vx, next_our_vy - opp_vy) > 120:
            ...
            if opp_target_d > 0 and bounce_d > 0:
                dot_product = (bounce_dx * opp_target_dx + bounce_dy * opp_target_dy) / (bounce_d * opp_target_d)
                # We only want to shield if the collision bumps them away from the next checkpoint (dot_product <= 0.0).
                if dot_product > 0.0:
                    should_shield = False
