...
def main():
    ...
    while True:
        ...
        # Proportional continuous thrust calculation
        if angle_to_target_abs > 90:
            thrust = 0
        else:
            angle_factor = max(0.0, 1.0 - (angle_to_target_abs / 90.0))
            thrust = int(100.0 * angle_factor)

        # Proportional corner braking
        ...
        # Behind-Mode: Trailing blocker goes into aggressive intercept/pursuit mode
        we_are_trailing = (score_opp_runner > score_our_runner + 1500)
        
        if we_are_trailing and opp_dist < 3500:
            # Aggressively target the opponent runner's future position
            target_x = opp_runner.x + opp_runner.vx * 1.5
            target_y = opp_runner.y + opp_runner.vy * 1.5
            thrust = 100
        else:
            # Standard Positioning / Camping Post
            if dist_to_post > 150:
                # Travel back to post using proportional thrust to prevent overshooting
                target_x = post_x
                target_y = post_y
                target_speed = min(100.0, dist_to_post * 0.4)
                thrust = int(min(100.0, max(20.0, target_speed - blocker.speed)))
            else:
                # At the post: face the approaching opponent and sit steady
                target_x = opp_runner.x + opp_runner.vx
                target_y = opp_runner.y + opp_runner.vy
                thrust = 0 if blocker.speed > 20 else 10

            # Override: Intercept and ram the opponent runner when they get close to checkpoint/post
            if opp_dist < 2000:
                target_x = opp_runner.x + opp_runner.vx * 1.5
                target_y = opp_runner.y + opp_runner.vy * 1.5
                thrust = 100
        ...
        # Avoid team collisions with runner
        for t in [1, 2, 3]:
        ...
        # More aggressive shielding for blocker offensive rams
        for opp in opp_pods:
            ...
            if collide and blocker.state.shield_cooldown == 0:
                ...
                # Lowered trigger threshold to 80 for offensive collisions
                if rel_speed > 80:
                    thrust = "SHIELD"
